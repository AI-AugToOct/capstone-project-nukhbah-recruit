...
def match_candidates(cvs_data, job_description: str, job_field: str, output_path: str = "qualified_candidates.json"):
    ...
    for cv_data in cvs_data:
        ...
        # Technical skills
        skills_dict = cv_data.get("technical_skills", {})
        skills_list = []
        ...
        # Projects
        projects_list = [f"{proj.get('name','')} {proj.get('description','')}" for proj in cv_data.get("projects", [])]
        ...
        if final_similarity >= SIMILARITY_THRESHOLD:
            qualified_candidates.append({
                "id": candidate_id, #   لازم ناتكد من اسم id
                "full_name": candidate_name,
                "email": cv_data.get("contact", {}).get("email", ""),
                "phone": cv_data.get("contact", {}).get("phone", ""),  # إضافة رقم الجوال
